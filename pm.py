...
def get_password():
    ...
    otp = generate_otp()
    sent = send_otp_via_gmail(email, otp)
    if not sent:
        messagebox.showerror("Email Failed", "OTP couldn't be sent. Try again.")
        return

    user_otp = simpledialog.askstring("OTP Verification", f"Enter OTP sent to {email}:")
    if user_otp != otp:
...
root = tk.Tk()
root.title("🔐 Password Manager (with OTP Verification)")
